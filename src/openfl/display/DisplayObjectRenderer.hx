			...
			if (softwareDirty || hardwareDirty)
			{
				...
				if (renderType == OPENGL)
				{
					...
					var updateTransform = (needRender
						|| !displayObject.__cacheBitmap.__worldTransform.equals(displayObject.__worldTransform));
					...
					if (needRender)
					{
						...
						if (displayObject.__cacheBitmapRenderer == null || renderType != displayObject.__cacheBitmapRenderer.__type)
						{
							if (renderType == OPENGL)
							{
								displayObject.__cacheBitmapRenderer = new OpenGLRenderer(cast(renderer, OpenGLRenderer).__context3D,
